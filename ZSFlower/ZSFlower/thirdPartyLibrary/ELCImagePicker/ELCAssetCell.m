...
- (void)setAssets:(NSArray *)assets
{
    ...
    for (int i = 0; i < [_rowAssets count]; ++i) {
        ...
        if (i < [_overlayViewArray count]) {
            ...
            if (overlayImage == nil) {
                overlayImage = [UIImage imageNamed:@"personal__address_red"];
            }
...
- (void)cellTapped:(UITapGestureRecognizer *)tapRecognizer
{
	...
	CGRect frame = CGRectMake(startX, 2, (SCREEN_WIDTH-20)/4, 75);
...
- (void)layoutSubviews
{
    ...
    CGRect frame1 = CGRectMake(startX+(SCREEN_WIDTH-20)/4-19, 58, 15, 15);
	...
	for (int i = 0; i < [_rowAssets count]; ++i) {
        ...
        [overlayView setFrame:frame1];
        [self addSubview:overlayView];
		
		frame.origin.x = frame.origin.x + frame.size.width+4;
        
        frame1.origin.x +=  frame.size.width+4;
	}
}
